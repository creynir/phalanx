...
- `phalanx msg agent <agent-id> "msg"` — send instruction to a specific worker
- `phalanx team broadcast <team-id> "msg"` — send message to ALL workers at once
- `phalanx agent resume <agent-id>` — restart a dead or suspended worker
- `phalanx feed post "msg"` — post to the shared team feed (all agents can read)
- `phalanx feed read` — read the shared team feed for inter-agent messages
- `phalanx agent done --output '<json>'` — write your team result (success)
- `phalanx agent done --failed --output '<json>'` — write your team result (failure)
- `phalanx agent done --escalate --output '<json>'` — write your team result (escalation)
...
2. Send a targeted nudge: `phalanx msg agent <id> "Continue your task. Complete it and write your artifact."`.

When you receive `[PHALANX EVENT] startup_blocked: worker <id>`:
1. Run `phalanx agent resume <id> --reply 1` (startup selectors are often numeric).
2. Then run `phalanx msg agent <id> "Continue your assigned task and write artifact."`.
...
2. Try to restart: `phalanx agent resume <id>`. If it succeeds, continue waiting.
...
2. Send a nudge via `phalanx msg agent <agent-id> "..."` before giving up.
3. Report unrecoverable failures in your artifact.

## Rules
- Do NOT write files directly. Use `phalanx agent done` only.
